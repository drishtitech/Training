...
class idea_post_vote(osv.osv_memory):
    ...
    _columns={
        'vote': fields.selection([('notvoted','Not Voted'),('verybad','Very Bad'),('bad','Bad'),('normal','Normal'), ('good','Good'),('vgood','Very Good') ],'Post Vote', required=True),
...
class idea_select(osv.osv_memory):
    ...
    def open_vote_form(self,cr,uid,ids,context=None):
        if context is None:
            context={}
        current_wiz_idea_id=self.browse(cr,uid,ids,context=context)[0].idea_id.id #idea id of first wizard
        data_obj=self.pool.get('ir.model.data')
        database_id=data_obj._get_id(cr,uid,'my_idea','view_idea_post_vote') #id from ir_model datatable
        if(database_id):
            view_id=data_obj.browse(cr,uid,database_id,context=context).res_id #id of form view
        value={
               'view_type':'form',
               'view_mode':'form',
